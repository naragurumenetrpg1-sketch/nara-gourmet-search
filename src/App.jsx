...
export default function GourmetSearch() {
  ...
  // ジャンルに応じた色を返す関数
  const getGenreColors = (genreText) => {
    const genres = genreText.split(',').map(g => g.trim());
    const colors = [];

    genres.forEach(genre => {
      if (genre.includes('料理')) {
        colors.push('#e5e7eb'); // 薄灰色（さらに薄く）
      } else if (genre.includes('肉') || genre.includes('焼肉')) {
        colors.push('#fdba74'); // オレンジ（薄く）
      } else if (genre.includes('ラーメン') || genre.includes('つけ麺') || genre.includes('中華')) {
        colors.push('#fca5a5'); // 赤（薄く）
      } else if (genre.includes('カフェ') || genre.includes('スイーツ')) {
        colors.push('#67e8f9'); // 水色（薄く）
      } else if (genre.includes('パン')) {
        colors.push('#fde047'); // 黄色（薄く）
      } else {
        colors.push('#d8b4fe'); // 紫（薄く、その他）
      }
    });

    return colors.length > 0 ? colors : ['#d8b4fe']; // デフォルトは紫
  };
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
              {currentResults.map((store, idx) => {
                const colors = getGenreColors(store.genre);
                return (
                  <a
                    key={startIndex + idx}
                    href={store.link || '#'}
                    ...
                    className={`group bg-white border border-gray-200 rounded-lg overflow-hidden transition-all duration-500 hover:-translate-y-1 block relative ${
                  ...
                  >
                    ...
                    <div className="p-4 sm:p-6">
                      <div className="flex items-start justify-between mb-2 sm:mb-3">
                        <h3 className="text-base sm:text-lg md:text-xl font-medium text-black group-hover:text-gray-700 transition-colors duration-300 pr-4">
                          {store.name}
                        </h3>
